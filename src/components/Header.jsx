...
const Header = ({ showLogoVideo }) => {
  // 1. Initialize based on localStorage OR System Preference
  ...
  // 2. Listen for OS-level theme changes dynamically
  useEffect(() => {
    const mediaQuery = window.matchMedia("(prefers-color-scheme: dark)");
    
    const handleChange = (e) => {
      // Only auto-switch if the user hasn't manually clicked the toggle yet
  ...
  // 3. Apply the theme to the DOM without auto-saving to localStorage
  useEffect(() => {
    document.documentElement.setAttribute("data-theme", darkMode ? "dark" : "light");
  }, [darkMode]);

  // 4. Manual user toggle function
  const toggleTheme = () => {
    const newTheme = !darkMode;
    setDarkMode(newTheme);
    // Now we lock in the user's choice so it stops following the OS
      ...
      >
        ...
        >
          ...
          <div className="flex items-center gap-6">
            ...
            <li>
              <button
                onClick={toggleTheme} // <--- Updated to use custom function
                style={{
                  backgroundColor: "var(--bg-color)",
                  border: `1px solid var(--border-color)`,
                }}
